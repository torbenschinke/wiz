...
| #2 |16|ndptr| vtable | * | reference to the virtual address table. If the offset (the last 8 byte) are 0x00, no virtual addresses are in use yet (and have never been used).
| #3 |16|ndptr| commit tree | *| an uncompressed 128 bit node pointer to the tree of named commits (tags or branches)
| #4 |16|ndptr| free space tree | *| an uncompressed 128 bit node pointer to the tree of free areas
| #5 |16|ndptr| reference count tree | *| an uncompressed 128 bit node pointer to the tree of reference counts
| #6 |32|hash| checksum |* | the hash of fields #0 - #5
|==============================================
