...
Note that depending on the chosen data and stream nodes are not hashed 
...
| #3 |'len' |[]uint8| message | *|  byte array containing a message payload. The format of this message is part of the application's domain and not specified, e.g. it may just be an utf8 string.
| #4 |1-*|varuint|count|*|Amount of trees in this commit as count
| #5 |'count'*([]uint8 + hash)|[]named tree|trees|*|sorted list of trees, each with a byte array as name, followed by it's hash.
|==============================================

TODO: separate merkle tree in the transaction seems not to make sense.
